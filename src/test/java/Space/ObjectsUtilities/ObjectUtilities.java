package Space.ObjectsUtilities;
...
//	}
	...
	public void clickElement(String xpath) throws ElementNotLocatedOnUIException {
		Reporter__log("Attempting to click..");
		int timeoutInSec=	Setup.defaultTimeOutForObjectWait;
	...
	public void waitUntilElementIsVisible1(String xpath, int timeoutInSec) {
		try {
			// Reporter__log("waitUntilElementIsVisible: For element with xpath \"" + xpath
			// + "\"");
			@SuppressWarnings("deprecation")
			...
			// Reporter__log("<div style='color:orange'><b>WARNING:</b><br />" +
			// e.getMessage() + "</div><br />");
		}
	}

	public void clickElementByJS1(WebElement element) throws Exception {

		if (waitForObject(element, 30)) {
			JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].click();", element);
			try {
	...
	public void TypeByJs(String content) {
		WebElement wb = driver.findElement(By.xpath(content));
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].value='';", wb);

	}

	public void clearbyJS(String xpath) {
		driver.findElement(By.xpath(xpath));
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].value='';", xpath);
	...
	public void Actions(String xpath, String xpath1) {
		...
		actions.moveToElement(abc).click();
		actions.moveToElement(driver.findElement(By.xpath(xpath1))).click().perform();
	}
	...
	public void assertVerify(String xpath,String VerifyText) {
		@SuppressWarnings("deprecation")
		WebDriverWait wait = new WebDriverWait(driver, 300);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath)));
		String Text = driver.findElement(By.xpath(xpath)).getText();
		System.out.println(Text + VerifyText );
		Assert.assertTrue((VerifyText.equalsIgnoreCase(Text)), "Test Verification Failed");
		Reporter.log("Test Verification Passed");
	}
	public void selectItemFromDropdown1(String name, String content, boolean byVisibleText, boolean byValue,
			int byIndex) throws ElementNotLocatedOnUIException {
		selectItemFromDropdown1(name, content, byVisibleText, byValue, byIndex, Setup.defaultTimeOutForObjectWait);
	}
	...
	public void selectItemFromDropdown1(String name, String content, boolean byVisibleText, boolean byValue,
		...
		if (waitForObject(name, timeoutInSec)) {
			name ="name";
			...
			if (byVisibleText)
				...
				drp.selectByIndex(byIndex);
			Reporter__log("Selected successfully item: " + content + " from dropdown: \"" + name + "\"");
		} else
			throw (new ElementNotLocatedOnUIException(name));
	}
	public String getTextHidden(String xpath) {
		WebDriverWait wait = new WebDriverWait(driver, 30);
		
	    // Find the hidden element by its ID (you may need to change this according to your HTML structure)
        WebElement hiddenElement = (wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))));

        // Use JavaScript to get the inner text of the hidden element
        String hiddenText = (String) ((JavascriptExecutor) driver).executeScript("return arguments[0].value;", hiddenElement);

        // Print or use the captured hidden text
        System.out.println("Hidden Text: " + hiddenText);
		return hiddenText;
	}
	public void waitUntilElementIsClickable(String xpath, int timeoutInSec) {
		try {
			Reporter__log("waitUntilElementIsVisible: For element with xpath \"" + xpath + "\"");
			@SuppressWarnings("deprecation")
			WebDriverWait wait = new WebDriverWait(driver, timeoutInSec);
	...
	public void loader() throws IOException {
		String xpath_elem_ajaxLoaderSmall = prop.GetdataFromPropFileAM("elem_loader");
		Reporter__log("Waiting for smallLoader to appear");
		waitUntilElementIsVisible(xpath_elem_ajaxLoaderSmall, 10);
		Reporter__log("Waiting for smallLoader to disappear");
		waitUntilElementNotVisible(xpath_elem_ajaxLoaderSmall, 10);
}
